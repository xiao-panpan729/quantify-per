...
# ★回顾窗口：读上个窗口最后几轮纯问答 + session_log 摘要
#   方式: python tools/read_previous_session.py
#          python tools/read_previous_session.py --rounds 10
#          python tools/read_previous_session.py --list       # 列出所有会话
#          python tools/read_previous_session.py --no-session-log  # 跳过 session_log
#   原理: 扫描 ~/.claude/projects/*.jsonl → 按 mtime 排序 → 跳过最新（当前会话）
#        取上一个 → 提取 user/assistant 的 text block → 显示最后 6 轮
#        然后自动读取 session_log/ 下最新的 .md 摘要（按文件名排序，最新日期=最新摘要）
#   注意: 跳过 <ide_ 噪音、跳过 assistant 的 thinking/tool_use block
# S级必查：
...
python tools/event_ic_backtest.py                # ★事件流IC衰减回测(全量)
python tools/event_ic_backtest.py --chain 功率半导体  # 单链回测
...
python tools/event_stream.py                      # ★事件流框架—跨专家统一输出协议(增量JSON+生命周期)
python tools/event_stream.py --help               # 查看用法
python tools/backfill_events.py                   # ★事件流历史回填(从知识星球→事件)
python tools/backfill_events.py --days 90         # 回填最近90天
python tools/backfill_events.py --chain 功率半导体  # 只匹配指定产业链
...
# ─── ★午后产业分析管道（多源→行业事件推导→Obsidian知识库+GitHub） ───
python tools/afternoon_pipeline.py                  # 全量: 一思一记/猫菲特/盘前/知识星球/IMA→行业事件推导→个股
python tools/afternoon_pipeline.py --no-git         # 不推送GitHub
python tools/afternoon_pipeline.py --dry-run        # 只显示输入预览
python tools/afternoon_pipeline.py --date 20260628  # 指定日期回填
python tools/stock_profiles.py --stat               # ★个股说明知识库统计
python tools/stock_profiles.py --search 芯联集成     # 查个股说明
python tools/stock_profiles.py --all                # 全量导出（按板块分组）
python _publish_industry.py                          # 把产业分析发布到 GitHub Pages（daily-reports）
update_afternoon.bat                                 # ★全流程: 知识星球→IMA→公众号归档→产业分析→Obsidian+Pages
# 输出: D:/knowledge-hub/industry_daily/ + D:/knowledge-hub/wechat_daily/ + reports/industry_daily/
python tools/clean_history_to_industry.py             # ★历史文章→产业报告格式(盘前纪要规则提取+一思一记LLM)
python tools/clean_history_to_industry.py --dry-run   # 预览不写入
python tools/clean_history_to_industry.py --source 盘前纪要  # 只跑指定来源
...
# ─── IMA 知识库截获 + OCR（mitmproxy + 腾讯云OCR高精度版） ───
mitmdump -s tools/ima_capture.py -p 8888          # ★启动中间人代理，截获 IMA 共享知识库 + 知识星球图片
python tools/ocr_ima_kb.py                        # ★增量OCR：扫描未转图片 → 腾讯云OCR → Obsidian MD
python tools/ocr_ima_kb.py --date=20260628        # 只处理指定日期
python tools/ocr_ima_kb.py --dry-run              # 预览待处理图片
python tools/ocr_ima_kb.py --force                # 强制重转所有（跳过 MD5 去重）

# ─── ★抖音视频 → ASR 语音转文字流水线 ───
python tools/douyin_asr_pipeline.py URL                    # 下载+ASR一条龙
python tools/douyin_asr_pipeline.py URL --dry-run          # 只下载不ASR
python tools/douyin_asr_pipeline.py --audio audio.mp3      # 只ASR已有音频
python tools/douyin_asr_pipeline.py URL --speed 1.5        # 音频1.5倍加速（省33%额度）
python tools/douyin_asr_pipeline.py URL --speed 2.0        # 2倍加速（省50%额度）
# 输出: D:/knowledge-hub/douyin_asr/{YYYY-MM-DD}/@{author}_{title}.md（Obsidian知识库）
# 依赖: dy-cli / tencentcloud-sdk-python-asr / ffmpeg
# 需先开通: https://console.cloud.tencent.com/asr （录音文件识别，10h/月免费）
# 依赖: dy-cli / tencentcloud-sdk-python-asr / ffmpeg
# 需先开通: https://console.cloud.tencent.com/asr （录音文件识别，10h/月免费）
